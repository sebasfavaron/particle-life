...
| 15. WebGPU end-to-end | Implementado y validado en M3 Pro | Shaders WGSL, backend ping-pong con grilla atómica, render directo y adapter CPU→GPU. La UI principal selecciona WebGPU sólo tras iniciar correctamente; mantiene fallback CPU y canvases separados. | Pi: fallback `adapter-unavailable` verificado. M3 Pro / Metal: oráculo físico pasó matriz asimétrica, repulsión corta, wrap de una celda y rebote. CPU vs GPU, 10 ticks tras 2 warm-up y sin cola: 2k `1,74/0,50 ms` (3,48×); 10k `16,17/1,11` (14,57×); 20k `61,92/3,70` (16,74×); 30k `137,53/5,14` (26,76×). |
