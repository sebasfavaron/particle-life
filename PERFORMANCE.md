...
| 15. WebGPU end-to-end | En progreso: hito aislado listo | Shaders WGSL, backend con ping-pong + grilla atómica y render directo, y página aislada `webgpu.html`. Main UI sigue CPU + Canvas hasta validar GPU real. | Pi: fallback `adapter-unavailable` verificado sin errores; `npm test` pasa. `webgpu.html` expone ticks/s, submit p95 y demora de cola GPU para la medición Mac. Mac debe validar adapter, compilación, fuerzas asimétricas y rendimiento. No hay claim de performance GPU todavía. Plan: `/tmp/particle-life-webgpu-plan.md`. |
