...
test('lively defaults and 40 classes are supported',()=>{const s=new ParticleLife();assert.equal(s.count,2000);assert.equal(s.types,10);assert.equal(s.radius,80);assert.equal(s.force,.02);assert.equal(s.damping,.7);s.configure({types:40});assert.equal(s.types,40);assert.equal(s.matrix.length,1600);});
